        ...
        <main class="flex-1 overflow-y-auto p-6">
            ...
            <div class="mb-8" data-aos="fade-down">
                <div class="flex justify-between items-center flex-wrap gap-4">
                    ...
                    <button onclick="openTambahModal()"
                        class="btn-primary text-white px-5 py-2 rounded-xl font-semibold shadow-lg">
                        <i class="fas fa-plus mr-2"></i> Tambah Kategori
                    </button>
                </div>
                <div class="mt-4">
                    <input type="text" id="searchInput" placeholder="Cari kategori..." class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8" data-aos="fade-up">
                <div class="bg-white rounded-2xl p-5 shadow-lg stat-card">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-3xl font-extrabold text-gray-800 mt-1">{{ $totalKategori }}</p>
                        </div>
                        <div class="bg-amber-100 p-3 rounded-xl"><i class="fas fa-tags text-amber-600 text-xl"></i>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-2xl p-5 shadow-lg stat-card">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-gray-500 text-sm">Kategori Terpakai</p>
                            <p class="text-3xl font-extrabold text-gray-800 mt-1">{{ $kategoriDigunakan }}</p>
                        </div>
                ...
                <div class="bg-white rounded-2xl p-5 shadow-lg stat-card">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-gray-500 text-sm">Kategori Tidak Terpakai</p>
                            <p class="text-3xl font-extrabold text-gray-800 mt-1">
                                {{ $totalKategori - $kategoriDigunakan }}</p>
                        </div>
                        <div class="bg-gray-100 p-3 rounded-xl"><i class="fas fa-trash-alt text-gray-600 text-xl"></i>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden card-hover">
                        <div class="bg-gradient-to-r from-amber-500 to-amber-600 px-4 py-3 text-white">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="flex gap-2">
                                    <button onclick="openEditModal('{{ $k->id_kategori }}', '{{ $k->ket_kategori }}')"
                                        class="text-white/80 hover:text-white">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    @if ($jumlahPenggunaan == 0)
                                        <a href="/hapus-kategori/{{ $k->id_kategori }}"
                                            onclick="return confirm('Yakin hapus kategori {{ $k->ket_kategori }}?')"
                                            class="text-white/80 hover:text-white">
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex justify-between items-center mt-3 pt-3 border-t border-gray-100">
                                ...
                                @if ($jumlahPenggunaan > 0)
                                    <span class="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-full"><i
                                            class="fas fa-info-circle"></i> Sedang digunakan</span>
                                @else
                                    <span class="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-full"><i
                                            class="fas fa-trash"></i> Dapat dihapus</span>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <div id="tambahModal"
        ...
        <div class="bg-white rounded-2xl max-w-md w-full mx-4 p-6 modal-pop">
            <div class="flex justify-between items-center mb-4 border-b pb-3">
                <h3 class="text-xl font-bold gradient-text"><i class="fas fa-plus-circle mr-2"></i>Tambah Kategori
                </h3>
                <button onclick="closeTambahModal()" class="text-gray-400 hover:text-gray-600"><i
            ...
            <form action="/tambah-kategori" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="ket_kategori"
                        placeholder="Contoh: Kerusakan AC, Kerusakan Kursi, dll"
                        class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500"
                        required>
                </div>
                <button type="submit" class="w-full btn-primary text-white py-2 rounded-xl font-semibold">Tambah
                    Kategori</button>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 modal-backdrop hidden">
        <div class="bg-white rounded-2xl max-w-md w-full mx-4 p-6 modal-pop">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Nama Kategori</label>
                    <input type="text" id="editKategoriName" name="ket_kategori"
                        class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500"
                        required>
                </div>
